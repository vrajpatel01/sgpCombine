...
import { useGetGroupMembers, useGetGroupSelectionDate, useGetProjectDetails } from "../services/query";
...
export default function GroupInfoContextProvider({ children }) {
    ...
    useEffect(() => {
        if (groupSelectionDate.isSuccess) {
            const startDate = new Date(groupSelectionDate?.data?.data?.grSelectionStartDate)
            const endDate = new Date(groupSelectionDate?.data?.data?.grSelectionEndDate)
    ...
    return (
        <GroupInfoContext.Provider value={{
            projectDetails, dates, groupMembersInfo, isLoading: {
