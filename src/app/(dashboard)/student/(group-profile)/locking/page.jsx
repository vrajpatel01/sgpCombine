...
import {
  ...
  useGenerateCoverPage,
} from "../services/query";

export default function LockingPage() {
  const { projectDetails, dates, isLoading, groupMembersInfo } =
    useContext(GroupInfoContext);
    ...
    return (
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Verify your project and group details and lock it.
          </CardDescription>
        </CardHeader>
        <CardContent>
  ...
  if (
    projectError?.status == 500 ||
    projectError?.status == 404 ||
    groupError?.status == 500 ||
    groupError?.status == 404 ||
    datesError?.status == 500 ||
    datesError?.status == 404
  ) {
    return (
      <Card>
        <CardHeader>
        ...
        <CardContent className="py-20 border-t">
          <ErrorComp
            ...
            text="Having some issue please try again later"
          />
        </CardContent>
      </Card>
    );
    ...
    <div className="space-y-5 pb-5 overflow-hidden">
      <div className="space-y-5 overflow-hidden">
        ...
        <Card>
          ...
          <CardContent className="border-t px-6 py-4 text-justify overflow-x-scroll">
          ...
          <CardContent className="border-t px-6 py-4">
            <div className="space-x-2 space-y-2">
              {projectDetails?.data?.data?.technologies.map((item, i) => (
                <Badge
                  className="p-2 px-4 bg-secondary-foreground hover:bg-secondary-foreground select-none"
                  key={i}
                >
                  {item}
                </Badge>
          ...
          <CardContent className="border-t px-6 py-4">
            <div className="space-x-2 space-y-2">
              <DataTable
                checkbox={false}
                columns={columns}
                data={groupMembersInfo?.data?.students}
              />
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader>
            <CardTitle className="flex justify-start items-center gap-4">
              Locking{" "}
              <span>
                {projectDetails?.data?.data?.isLocked ? (
                  <Lock size={20} />
                ) : (
                  <LockOpen size={20} />
                )}
            ...
            <CardDescription>
              If you confirm, then you should lock your project submission form
              before end date. If you leave your locking deadline then your
              project is not submitted and you need meet your SGP professor for
              further guidance.
            </CardDescription>
          </CardHeader>
          {projectDetails?.data?.data?.isLeader &&
            !projectDetails?.data?.data?.isLocked &&
            dates.start &&
            dates.end && (
              <Form {...form}>
                        ...
                        <FormItem>
                          <FormControl>
                            <div className="flex items-center space-x-2">
                              ...
                              >
                                I have verified that the report mentioned below
                                is accurate.
                              </FormLabel>
                            </div>
                          </FormControl>
