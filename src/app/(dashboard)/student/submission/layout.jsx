...
export default function SubmissionLayout({ children }) {
  ...
  const inGroup =
    projectDetails?.error?.response?.status != 400 || !!projectDetails?.data;

  useEffect(() => {
    if (
      weekInfo.isSuccess &&
      parseInt(pathname.charAt(pathname.length - 1)) > currentWeek
    ) {
      return notFound();
    }
  }, [
  ...
  return (
    <SubmissionContextProvider>
      {error?.status == 401 ? (
        <ErrorComp icon={<CircleX size={42} />} text="Please try again later" />
      ) : !inGroup ? (
        <ErrorComp
          className="h-full"
