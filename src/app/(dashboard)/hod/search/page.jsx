...
export default function SearchPage() {
  ...
  const searchData = useSearchUser({
    search: search,
    searchBy: searchBy,
    role: role,
  });
  ...
  useEffect(() => {
    if (role && typeof role === "string") {
      router.replace(
        `/hod/search?role=${role}&search=${search}&by=${searchBy}`
      );
    }
  }, [role, searchBy, search]);
            ...
            {searchData.isSuccess &&
              searchData.data.data.map((user) => (
                <TableRow key={user.id}>
                  ...
                  {faculties ? (
                    <TableCell content={user?.mobileNumber} />
                  ) : null}
                  {faculties ? (
                    <TableCell content={user?.employeeCode} />
                  ) : null}
