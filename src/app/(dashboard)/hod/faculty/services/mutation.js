...
import { addFaulty, assignCoordinator, createAccountWithCSV, deassignCoordinator, deleteFacultyAccount, deleteMultipleFacultiesAccount, editFacultyAccount } from "./api";
...
export const useAssignCoordinator = () => {
    ...
    return useMutation({
        ...
        onSettled: async (data, error, variables) => {
            toast.success(data.message)
            if (data.success === true) {
                await queryClient.invalidateQueries({ queryKey: ['faculty', variables.id] })
            }
        }
    })
}

export const useDeassignCoordinator = () => {
    const queryClient = useQueryClient()

    return useMutation({
        mutationFn: (data) => deassignCoordinator(data),
        onError: (error) => {
            if (error instanceof AxiosError) {
                return toast.error(error.response.data?.message)
            }
