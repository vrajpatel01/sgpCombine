...
export function CoordinatorModel({ currentUserData, setCoordinatorModel }) {
    ...
    useEffect(() => {
        if (currentUserData.coordinatorSem?.length > 0) {
            setSelectedSemester(currentUserData.coordinatorSem)
        } else {
            setSelectedSemester([])
        }
    }, [currentUserData.coordinatorSem])
            ...
            <div className="grid grid-cols-8 gap-2">
                {Array(8).fill(0).map((_, index) => (
                    <Button key={index} className={cn({
                        '!bg-black/30 text-black': selectedSemester?.includes(index + 1),
                    })} onClick={() => addSemester(index + 1)} variant='secondary'>{index + 1}</Button>
                ))}
