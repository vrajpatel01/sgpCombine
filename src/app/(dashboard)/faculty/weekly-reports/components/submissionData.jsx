  ...
  if (groupData.isPending) {
    return (
      <div className="h-screen w-full flex justify-center items-center">
        <div className="border-black border-l-2 rounded-full w-14 h-14 animate-spin"></div>
      </div>
    );
  }
  const url = pathname
    .split("/")
    .filter((path, index) => index !== 4)
    .join("/");
      ...
      <div className="w-full h-full flex justify-center items-center flex-col">
        <div className="flex justify-between sm:justify-start items-center gap-3 w-full">
          <div className="flex justify-between items-center">
            <Link
              href={url}
              className="p-2 rounded-full hover:bg-border text-center cursor-pointer"
            >
              <span>
                <IoChevronBackSharp size={18} />
              </span>
            </Link>
            <h1 className="text-title-28 whitespace-nowrap flex flex-nowrap">
              Week {currentWeek}
            </h1>
          </div>
        </div>
    ...
    <div className="w-full space-y-5">
          ...
          <div className="flex justify-between items-center">
            ...
            <h1 className="text-title-28 whitespace-nowrap flex flex-nowrap">
              ...
              <span className="hidden sm:block">
                {" "}
                - {new Date(groupData.data?.data?.updatedAt).toDateString()}
              </span>
            </h1>
          </div>
          <span className="capitalize border-border border-1 select-none px-3 py-2 rounded-md">
            {groupData.data?.data?.status}
          </span>
        </div>
        {groupData.data.data.isLocked &&
          !(groupData.data?.data?.status === "approved") && (
                ...
                <Dialog>
                  ...
                  <DialogContent>
                    <DialogHeader>
                      ...
                      <DialogDescription>
                        Are you sure you want to accept this weekly report.
                        after accepting this report you can not able to do any
                        changes.
                      </DialogDescription>
      ...
      <Tabs defaultValue="overall">
        <TabsList>
          ...
          <TabsTrigger
            disabled={groupData?.data?.data?.rejectMessages.length === 0}
            value="rejection"
          >
            Rejection
          </TabsTrigger>
